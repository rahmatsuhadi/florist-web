...
export const Sidebar: React.FC<{ shopInfo?: StoreSettingsData }> = ({ shopInfo }) => {
  ...
  const websiteItems = [
    ...
    { id: "locations", path: "/admin/locations", label: "Lokasi & Ongkir", icon: MapPin },
  ];
  ...
  return (
    <>
      ...
      <div 
        className={`
          w-64 h-screen bg-white/90 md:bg-white/60 backdrop-blur-xl border-r border-brand/10 
          fixed left-0 top-0 flex flex-col z-50 shadow-[4px_0_24px_rgba(0,0,0,0.01)]
          transition-transform duration-300 ease-in-out
          ${isOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}
        `}
      >
        <div className="p-8 pb-6 flex items-center justify-between">
          <h2 className="font-serif text-2xl font-bold text-brand tracking-wide truncate">
            {shopName}.
          </h2>
          <button 
            onClick={closeSidebar}
            className="md:hidden p-2 text-gray-400 hover:bg-gray-100 hover:text-gray-600 rounded-lg transition-colors"
          >
            <X size={20} />
          </button>
        </div>
        ...
        <div className="p-4 border-t border-brand/10">
          <button className="flex items-center gap-3 w-full p-3 rounded-2xl hover:bg-gray-50 transition-colors text-left">
            <div className="w-10 h-10 rounded-full bg-[#B76E79]/20 flex items-center justify-center text-[#B76E79] font-serif font-bold shrink-0">
              A
            </div>
            <div className="flex-1 truncate">
              <p className="text-sm font-semibold text-gray-800 truncate">Admin Utama</p>
