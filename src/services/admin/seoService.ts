...
export const getSeoSettings = unstable_cache(
  async (pageName: string, resolveVariables: boolean = false): Promise<SeoSettingsData> => {
    try {
      ...
      // If db not available (build-time), use defaults
      if (!db) {
      ...
      // Return basic defaults instead of throwing
      return {
        pageName,
        title: "Florist | Rangkaian Bunga Premium",
        description: "Toko bunga premium dengan rangkaian bunga segar berkualitas tinggi.",
        keywords: "toko bunga, florist, buket bunga"
      };
    }
  },
  ["seo-settings"],
