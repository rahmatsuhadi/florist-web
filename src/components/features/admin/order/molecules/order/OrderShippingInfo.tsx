    ...
    <div className="bg-white p-6 rounded-2xl border border-brand/10 shadow-sm space-y-6 h-full">
      ...
      <div className="space-y-4 pt-1">
        <div className="flex items-start gap-4">
          <div className="p-2.5 bg-brand-light text-brand rounded-xl mt-0.5"><MapPin size={20} /></div>
          <div>
            <h5 className="font-semibold text-gray-900 text-base">
              {/* @ts-ignore */}
              {transaction.deliveryMethod === "pickup" ? "Metode Pengambilan" : "Alamat Pengiriman"}
            </h5>
            <p className="text-sm text-gray-500 mt-1.5 leading-relaxed font-sans">
              {/* @ts-ignore */}
            ...
            {transaction.deliveryMethod !== "pickup" && transaction.customerLatitude && transaction.customerLongitude && (
              <div className="mt-3 space-y-3">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    <MapPin size={14} /> {showMap ? "Tutup Peta" : "Lihat Peta Tersemat"}
                  </button>
                  <a
                    // @ts-ignore
                    ...
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-brand/10 hover:bg-brand/20 text-brand text-xs font-bold rounded-lg transition-all"
                  >
        ...
        {(transaction.scheduledDate || transaction.scheduledTime) && (
          <div className="flex items-start gap-4 border-t border-gray-100 pt-5 mt-2">
            <div className="p-2.5 bg-blue-50 text-blue-600 rounded-xl mt-0.5"><Calendar size={20} /></div>
            <div>
              <h5 className="font-semibold text-gray-900 text-base">
                {/* @ts-ignore */}
                {transaction.deliveryMethod === "pickup" ? "Jadwal Pengambilan" : "Jadwal Pengiriman"}
              </h5>
              <p className="text-sm text-gray-600 mt-1.5 font-sans space-y-1">
                <span className="block"><span className="font-semibold text-gray-800">Tanggal:</span> {/* @ts-ignore */} {transaction.scheduledDate || "-"}</span>
                <span className="block"><span className="font-semibold text-gray-800">Jam:</span> {/* @ts-ignore */} {transaction.scheduledTime || "-"}</span>
              </p>
            </div>
          </div>
        )}
